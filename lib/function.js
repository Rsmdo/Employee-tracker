...
addEmployee = () => {
 //get information from inputs then organize and release outputs to databases
  ...
  connection.query(sql, (err, rows) => {
    ...
    const getManager = function (managerName) {
      const sql = `SELECT employees.id FROM employees WHERE CONCAT(employees.first_name," ", employees.last_name) = "${managerName}"`;

      connection.query(sql, (err, rows) => {
        if (err) throw err; 
        //help reference correct manager incase there is the same name 
        let managerId = rows[0].id;
        //manager is established now need to add employee

        let employee = function (managerId) {
              ...
              .prompt([
                {
                  type: "input",
                  name: "firstName",
                  message: "(REQUIRED) Enter employee first name?",
                  validate: (input) => {
                    if (input) {
                      return true;
                    } else {
                ...
                },
                {
                  type: "input",
                  name: "lastName",
                  message: "(REQUIRED) Enter employee last name",
                  validate: (input) => {
                    if (input) {
                      return true;
                    } else {
